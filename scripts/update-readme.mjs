...
function renderCatalog(manifest) {
  ...
  for (const subject of manifest.subjects || []) {
    ...
    for (const [role, assets] of grouped.entries()) {
      ...
      if (assets.length === 1) {
        lines.push(
          `- ${role}: [${selected.title}](${encodePath(selected.publicPath)})`,
        );
      } else {
        const roleDir = `${subject.name}/${role}/`;
        lines.push(
          `- ${role}: ${assets.length} 个文件，见 [${role}/](${encodePath(roleDir)})。`,
        );
      }
    }
    lines.push("");
...
function main() {
  ...
  if (checkOnly) {
    if (nextReadme !== readme) {
      console.error(
        "README.md generated catalog is out of date. Run: node scripts/update-readme.mjs",
      );
